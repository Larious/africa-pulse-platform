...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; font-family:Inter, ui-sans-serif, system-ui, sans-serif; background:var(--paper); color:var(--ink); }
    ...
    .pill.failed { background:#fee4e2; color:var(--red); } .pill.partial { background:#fff4d6; color:var(--amber); } .empty { padding:24px; color:var(--muted); background:white; border:1px solid var(--line); }
...
<body>
  ...
  <script>
    ...
    async function load() {
      ...
      document.querySelector('#runs').innerHTML = table(data.runs, [{key:'source_id',label:'Source'}, {key:'status',label:'Status',render:v=>`<span class="pill ${v === 'failed' ? 'failed' : v === 'partial' ? 'partial' : ''}">${escape(v)}</span>`}, {key:'records_received',label:'Received'}, {key:'records_inserted',label:'Inserted'}, {key:'started_at',label:'Started'}]);
