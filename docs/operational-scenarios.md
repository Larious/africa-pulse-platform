...
An actual Overpass HTTP 406 failure and successful POST recovery remain in `control.ingestion_run`. The failed run inserted no commercial facts. Preserve the failed run, correct the connector, then rerun; do not replace missing values with invented observations.
